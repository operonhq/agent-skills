...
The `source` parameter is hardcoded for the LobeHub variant. Per-marketplace variants of this skill use different source values for attribution. Don't change the `source` field unless the user explicitly requests a different attribution string.
...
- `OPERON_SOURCE` (set to `skill3-lobehub` for the LobeHub variant)
...
2. Watch your placement log fill (visit [operon.so/developers](https://operon.so/developers?utm_source=skill3-lobehub&utm_medium=skill&utm_campaign=skills-distribution) for the dashboard).
...
- The `source` field is `skill3-lobehub` in the LobeHub variant. Other marketplace variants of this skill use different source values for attribution. Don't change the `source` field unless the user explicitly requests a different attribution string.
...
- [operon.so/developers](https://operon.so/developers?utm_source=skill3-lobehub&utm_medium=skill&utm_campaign=skills-distribution): publisher dashboard, registration, full SDK reference.
